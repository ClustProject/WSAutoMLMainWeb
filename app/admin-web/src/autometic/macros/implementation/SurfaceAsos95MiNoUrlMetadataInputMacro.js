...
class SurfaceAsos95MiNoUrlMetadataInputMacro extends NoUrlMetadataInputMacro {
  ...
  async run() {
    ...
    await this.selectDropDown(2);
    await this.selectDropDown(1);
    await this.typeText("종관기상관측이란 종관규모의 날씨를 파악하기 위하여 정해진 시각에 모든 관측 소에서 같은 시각에 실시하는 지상관측을 말합니다. 종관규모는 일기도에 표현되어 있는 보통의 고기압이나 저기압의 공간적 크기 및 수명을 말하며, 주로 매일의 날씨 현상을 뜻합니다.");
    await this.typeText("기상강원지방기상청 춘천기상대에서 측정한 데이터로 철원지점 해발고도 155m 지점에서 측정된 자료");
